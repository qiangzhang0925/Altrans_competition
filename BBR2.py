...
class MySolution(Packet_selection, Reno):
    ...
    def select_packet(self, cur_time, packet_queue):
        ...
        def is_better(packet):
            ...
            best_size = best_packet.offset * 1480 / best_packet.block_info["Size"]
            packet_size = packet.offset * 1480 / packet.block_info["Size"]
            ...
            if (cur_time - best_block_create_time) >= best_packet.block_info["Deadline"]:
                return True
             # 避免一个包因剩余长度太小而一直不被选择
            if best_block_remainedsize <= 0.1 * float(best_packet.block_info["Size"]) // 1480:
                return False
            if packet_remainedsize <= 0.1 * float(packet.block_info["Size"]) // 1480:
                 return True
            # 同一个block按序发送
            if best_packet.block_info["Block_id"] == packet.block_info["Block_id"]:
                ...
                if best_packet.block_info["Priority"] == packet.block_info["Priority"]:
                    return best_time_per > packet_time_per or best_size > packet_size
                else:
    ...
    def cc_trigger(self, data):
        ...
        if event_type == EVENT_TYPE_FINISHED:
            ...
            #排空阶段
                ...
                if self.inflight <= BDP or self.sp == 0:
